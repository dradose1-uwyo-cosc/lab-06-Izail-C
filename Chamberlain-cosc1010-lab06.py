...
# Sam Clackler
#ChatGPT to breakdown and clarify steps, define terms in depth 
...
"""
print(f"Total Characters In String:") #total count
...
# Step 1: Define the random string and remove newline characters
random_string = """
jppamiqxegokaizvkyawwurhewtcxohryzptznyuedhhmawpic
pkzwuiorngdfcsgqnlyifzyaivehpiyszykqprbcsobygzhadd
yfddbulxmcnyvqhesmnybyuhxjqqmhdxwhcselasiayqhctnlw
...
# Step 3: Count occurrences of each character using a dictionary
char_count = {}
for char in random_string:
    if char in char_count:
        char_count[char] += 1  # Increment count if character already in dictionary
    else:
        char_count[char] = 1   # Initialize count if it's the first occurrence

# Step 4: Sort dictionary by alphabetical order of characters
sorted_char_count = dict(sorted(char_count.items()))

# Step 5: Find the most and least occurred characters
...
# Step 6: Calculate the percentage of each character
total_chars = len(random_string)
char_percentage = {char: (count / total_chars) * 100 for char, count in sorted_char_count.items()}

# Step 7: Print occurrences and percentages
print("Character occurrences (sorted alphabetically):")
for char, count in sorted_char_count.items():
    print(f"Character: {char}, Occurrences: {count}, Percentage: {char_percentage[char]:.2f}%")

print("*" * 75)
print(f"The letter that occurred the most is '{most_occurred}' with {sorted_char_count[most_occurred]} occurrences.")
print(f"The letter that occurred the least is '{least_occurred}' with {sorted_char_count[least_occurred]} occurrences.")
